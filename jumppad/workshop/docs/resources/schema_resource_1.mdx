...
[{{url_scheme}}{{base_url}}:9090/redoc#tag/Schema]({{url_scheme}}{{base_url}}:9090/redoc#tag/Schema)
...
```shell
curl -H 'X-API-Key: supertopsecret' http://minecraft.container.jumppad.dev:9090/v1/schema/-1260/24/288/0 --data-binary @./schemas/car.zip
```
...
```shell
curl -H 'X-API-Key: supertopsecret' http://minecraft.container.jumppad.dev:9090/v1/schema/details/[id]
```
...
```shell
curl -H 'X-API-Key: supertopsecret' -XDELETE http://minecraft.container.jumppad.dev:9090/v1/schema/undo/[id]
```
...
Did you manage to complete that?

The Schema should look like the following example.
...
`*client`, which is  the go client for the API from this package. You can then 
update the configure block to look like the following example.
...
This is needed bbecause the API does not allow you to mutate a schema, 
...
`schemaPlanModifier{}` is a custom plan modifier that you will define later in
this chapter. It is used to check the hash of the file and force a replacement
should it change.
