...
func (p *MinecraftProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if apiKey == "" {
		resp.Diagnostics.AddError(
			"Configuration Error",
			"unable to set api_key, please set either the api_key property in the provider or the environment variable 'MINECRAFT_APIKEY'",
		)
		return
	}
...
<details>
  <summary>**Complete example:** provider.go</summary>
...
func (p *MinecraftProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	// set endpoint and apiKey from config if they are not null
	if !data.Endpoint.IsNull() {
		endpoint = data.Endpoint.ValueString()
	}
